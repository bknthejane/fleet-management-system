...
const UsersPage: React.FC = () => {
  ...
  // Filter users live when searchQuery or users change
  useEffect(() => {
    const query = searchQuery?.toLowerCase() || "";
    if (!query) {
      // Show all except system admins by default
      setFilteredUsers(users.filter((u) => !u.roleNames.includes("System Admin")));
    } else {
      setFilteredUsers(
        users.filter((u) => {
          // Exclude system admins always
          if (u.roleNames.includes("System Admin")) return false;

          // Check if any of the user fields match the search query
          return (
            u.userName.toLowerCase().includes(query) ||
            u.name.toLowerCase().includes(query) ||
            u.surname.toLowerCase().includes(query) ||
            u.emailAddress.toLowerCase().includes(query) ||
            u.roleNames.some((role) => role.toLowerCase().includes(query)) ||
            (u.municipalityName?.toLowerCase().includes(query) ?? false)
          );
        })
      );
    }
  ...
  const handleModalSubmit = () => {
    form.validateFields().then((values) => {
      ...
      if (modalType === "edit") {
        setUsers(
          users.map((u) =>
            u.id === currentUser.id ? { ...u, ...values, roleNames: u.roleNames, municipalityName: u.municipalityName } : u
          )
        );
